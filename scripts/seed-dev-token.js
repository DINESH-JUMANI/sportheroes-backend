...
async function main() {
  ...
  try {
    execSync(`node "${migrationScript}" 17.password_auth.sql`, { stdio: 'inherit', cwd: root });
  } catch {
  ...
  const accessToken = jwt.sign(
    {
      ...
      email: 'dev.tester@sportheroes.local',
    },
    jwtSecret,
    {
  ...
  console.log(`Password:      set via POST /auth/dev-login (DevPass123!)`);
